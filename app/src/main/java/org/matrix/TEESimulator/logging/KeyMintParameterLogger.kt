...
object KeyMintParameterLogger {
    private val algorithmNames: Map<Int, String> by lazy {
        Algorithm::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    private val ecCurveNames: Map<Int, String> by lazy {
        EcCurve::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    val blockModeNames: Map<Int, String> by lazy {
        BlockMode::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    val paddingNames: Map<Int, String> by lazy {
        PaddingMode::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    val purposeNames: Map<Int, String> by lazy {
        KeyPurpose::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    private val digestNames: Map<Int, String> by lazy {
        Digest::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    private val tagNames: Map<Int, String> by lazy {
        Tag::class.java.fields.filter { it.type == Int::class.java }
            .associate { field -> (field.get(null) as Int) to field.name }
    }
    ...
    fun logParameter(param: KeyParameter) {
        ...
        val formattedValue: String =
            when (param.tag) {
                ...
                Tag.AUTH_TIMEOUT, Tag.KEY_SIZE, Tag.MIN_MAC_LENGTH -> value.integer.toString()
                Tag.CERTIFICATE_SERIAL -> BigInteger(value.blob).toString()
                Tag.ACTIVE_DATETIME, Tag.CERTIFICATE_NOT_AFTER, Tag.CERTIFICATE_NOT_BEFORE,
                Tag.ORIGINATION_EXPIRE_DATETIME, Tag.USAGE_EXPIRE_DATETIME -> Date(value.dateTime).toString()
                ...
                Tag.ATTESTATION_CHALLENGE, Tag.ATTESTATION_ID_BRAND, Tag.ATTESTATION_ID_DEVICE,
                Tag.ATTESTATION_ID_PRODUCT, Tag.ATTESTATION_ID_MANUFACTURER, Tag.ATTESTATION_ID_MODEL,
                Tag.ATTESTATION_ID_IMEI, Tag.ATTESTATION_ID_SECOND_IMEI, Tag.ATTESTATION_ID_MEID,
