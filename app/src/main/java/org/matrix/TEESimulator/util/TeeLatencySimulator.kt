...
/**
 * Simulates realistic TEE key-generation latency so that attested operations never complete
 * faster than non-attested ones on the same device — a timing inversion that detectors score
 * against TEE coherence.
 *
 * The model mixes a log-normal base crypto delay (algorithm-dependent), an exponential
 * binder-transit component, Gaussian jitter, a per-session bias, and a one-time cold-start
 * penalty. The result is parked via LockSupport so the calling thread blocks for the
 * remaining time after the actual key generation completes.
 */
object TeeLatencySimulator {
    ...
    /**
     * Blocks the calling thread so the total elapsed time (generation + park) approximates
     * a realistic TEE delay for the given [algorithm].
     *
     * @param algorithm KeyMint algorithm constant (EC, RSA, AES, etc.)
     * @param elapsedNanos Nanoseconds already spent on the actual key generation.
     */
